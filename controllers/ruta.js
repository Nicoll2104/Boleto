...
const httpRutas = {
    ...
    getRutaid:  async(req, res)=>{
      ...
      postRuta: async (req, res) => {
        try {
          const { origen, destino, horarios, distancia, duracion} = req.body;
          const rutas = new ruta({ origen, destino, horarios, distancia, duracion });
    ...
    putRuta: async (req, res) =>{
        ...
        try{
            const rutas = await ruta.findByIdAndUpdate(id, {origen, destino, horarios, distancia, duracion}, {new: true});
